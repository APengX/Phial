...
Four files are dropped, covering four loading paths:
  ...
  - `.claude/skills/phial-graph/SKILL.md`
      Project-level skill for Claude Code. Triggered when the user asks
      to connect / relate the workspace's documents or build a knowledge
      graph. Follows the "LLM Wiki" pattern — reads the documents, finds
      real relationships, and wires them as links so they become edges in
      Phial's graph view.
...
GRAPH_SKILL_REL_PATH = Path(".claude") / "skills" / "phial-graph" / "SKILL.md"
...
- **[`.claude/skills/phial-graph/SKILL.md`](./.claude/skills/phial-graph/SKILL.md)** —— 项目级 skill。当用户想**在工作区的文档之间建立连接 / 构建知识图谱**时，用 `Skill` 工具调用 `phial-graph`：它按 LLM Wiki 模式读懂文档、找出真实关联、写成链接，让文档在 Phial 图谱里连起来。
...
GRAPH_SKILL_CONTENT = """---
name: phial-graph
description: Build and maintain a knowledge graph across the documents in this Phial workspace, the "LLM Wiki" way — read the documents, find the relationships that genuinely exist, and wire them as links so they appear as edges in Phial's graph view. Use when the user asks to connect / link / relate documents, build a knowledge graph or wiki, find connections between notes or papers, or 构建图谱 / 连接文档 / 把这些文档连起来 / 建立知识库. Goes beyond Phial's exact-title auto-link: it understands what each document is *about* and links related ones even when they never quote each other's titles.
---

# phial-graph — build the knowledge graph

You are inside a **Phial** workspace. Every `.html` file is one document, and
Phial draws a **graph** of them (the Graph view): nodes are documents, edges
are links between documents.

Phial's built-in "一键链接 / Link all" only connects documents that quote each
other's **exact title** — useless for a library of papers or notes that are
deeply related but never cite each other verbatim. Your job is the part a
string match can't do: **read the documents, understand them, and connect the
ones that genuinely belong together.**

This follows the **LLM Wiki** pattern: a knowledge base should not be
re-derived on every question — it is a persistent, interlinked artifact you
build once and keep current. The connections between documents are as
valuable as the documents themselves; the tedious bookkeeping of maintaining
them is exactly the work an LLM should own.

## The three layers in a Phial workspace

1. **Raw sources** — the user's `.html` documents. Their material. You connect
   and annotate it; you do not rewrite it.
2. **The text layer** — `.phial/text/*.md`, a plain-Markdown mirror of every
   document (regenerated by Phial's "提取文字 / Extract text"). **Read these to
   understand the whole workspace cheaply** instead of parsing every HTML
   file. It is a derived cache — never hand-edit it, it gets overwritten.
3. **The graph** — derived by Phial from the links between documents. You grow
   it by adding links.

## How a link becomes a graph edge

Phial draws an edge A→B when document A contains a link to document B:

```html
<a href="relative/path/to/B.html">…</a>
```

The `href` is relative to A's own path, points at a real `.html` file in this
workspace, and ends in `.html`. Save A and the graph shows the A→B edge. So
"connecting documents" literally means adding real `<a href>` links inside the
HTML.

## Operation: CONNECT (the main job)

1. **Survey.** Read `.phial/text/*.md` for every document — its title, what it
   is about, the recurring entities and concepts. If the text layer is missing
   or stale, ask the user to click "提取文字", or read the `.html` directly.
2. **Find the real relationships.** For each pair of documents ask: does one
   *build on*, *compare with*, *contradict*, *exemplify*, or *share a core
   topic* with the other? Connect those. Ignore coincidental word overlap — a
   sparse honest graph beats a dense noisy one.
3. **Add the link.** For each relationship A→B, edit A's `.html`:
   - Best: find the first natural in-prose mention of B's topic and wrap that
     phrase in `<a href="…B.html">`.
   - If there is no natural mention, append a small **"相关文档 / Related"**
     block near the end of `<body>` listing links to the related documents.
   - Keep edits minimal and on-style. Prefer small SEARCH/REPLACE-style edits
     over rewriting the document. Honor the HTML contract in `AGENTS.md`.
4. **Save & refresh.** Write each changed `.html` back into the workspace.
   Then tell the user to click "提取文字" (or "一键链接", which refreshes it
   automatically) so the text layer re-syncs and the graph picks up the new
   edges. Report which documents you linked and why, as a short list — do not
   paste HTML.

## Operation: SYNTHESIZE (optional hub pages)

A graph of links is good; a graph with **hubs** is better. When a cluster of
related documents emerges, offer to create new Phial documents that tie them
together — each is just another `.html`, so it becomes a node too:

- an **Overview / index** document linking to every document with a one-line
  summary, grouped by theme;
- **Concept / entity pages** — one document per recurring concept, linking out
  to every source that touches it.

These are LLM-authored documents. Build them as proper Phial HTML (see
`AGENTS.md`) and link them densely — they should be the hubs of the graph.

## Operation: LINT

On request, health-check the graph: orphan documents with no links in or out;
clusters that should be connected but are not; a concept mentioned across many
documents yet lacking its own page; stale links pointing at renamed or deleted
documents. Report findings and offer fixes.

## Rules

- A graph edge is only ever a real, relative, `.html`-targeted `<a href>` —
  that is the single thing Phial's graph reads.
- Don't invent relationships to inflate the graph.
- Don't rewrite the user's content — connect, annotate, and add new hub
  documents, but leave their material intact.
- Every document you create or edit must still satisfy Phial's HTML contract
  in `AGENTS.md` (self-contained, no external resources, Phial's visual
  language, the `window.phial` bridge).
- `.phial/text/` is read-only scratch for you — never rely on hand-edits there
  surviving the next extraction.
"""
...
# the log: AGENTS.md is the "anchor" the others reference, write it first.
_FILES: List[Tuple[Path, str]] = [
    ...
    (GRAPH_SKILL_REL_PATH, GRAPH_SKILL_CONTENT),
]
