...
from .html_style_reference import REFERENCE_DOC
...
6. 视觉风格按下面的「Phial 设计语言」来；适配窄屏。
7. 直接输出 HTML 本身（可以放在 ```html 代码块里，也可以裸输出）；不要在 HTML 之外写多余的解释。

## Phial 设计语言（文档的默认外观）

除非用户明确要求别的风格，生成的文档统一套用这套**温暖、克制、editorial** 的视觉语言。提示里随附了一篇《风格参考文档》——它本身就是一篇合格的范例，新建文档时照着它的 `:root` 变量和组件样式来。

- **配色**：背景象牙白 `#faf9f5`，卡片 / 面板纯白 `#ffffff`，柔和分区底 `#f0eee6`，填充色块用燕麦色 `#e3dacc`。正文文字石板黑 `#141413`、次要 `#3d3d3a`、更弱的元信息 `#87867f`，边框 `#d1cfc5`。
- **强调色**：陶土橙 `#d97757`（更深的 `#b85c3e`）——**只用在小处**：左边线、折叠三角、链接、focus、关键数字，不要大面积铺。语义色：成功橄榄绿 `#788c5d`、警告古铜 `#c78e3f`、危险铁锈红 `#b04a3f`、信息石板蓝 `#6a8caf`。**不要用冷蓝、纯黑、霓虹色或大面积渐变。**
- **字体**：标题用衬线体（`Georgia, "Songti SC", "Source Han Serif SC", serif`，字重 500，`letter-spacing: -0.01em`）；正文用系统无衬线（`-apple-system, "PingFang SC", "Microsoft YaHei", sans-serif`，15px、行高 1.6+）；代码、小标签、元信息（时间 / 计数 / 路径）用等宽，小标签可大写 + 加字间距。
- **形状**：圆角 10–12px，胶囊标签 / 徽章 999px。卡片 / 折叠块用 `1.5px solid #d1cfc5` 描边、或柔和阴影 `0 4px 14px rgba(20,20,19,.08)`，二者不要叠加。
- **结构**：正文型文档**开头必须放一个「30 秒速读」卡片**（陶土色左边线的浅色卡，两三句话点明「是什么 / 给谁看 / 看完能干嘛」）；正文段落限宽 ≤680px；多用 `<details>` 折叠、并排分栏、提示框来承载层级，别堆大段文字；长文档可加一条 sticky 侧边目录。
- **常驻面板必须可折叠**：只要你放了 sticky / fixed 的侧边目录、侧栏、底部工具条等会一直占着位置的面板，就**必须给它一个收起 / 展开的小按钮**（默认展开；收起后只留一个能再点开的小把手 / 窄条）。文档渲染在一个较窄的预览区里，这些常驻面板不能一直挡住正文——用一个 class 切换 + 几行 JS 实现即可，别让它们无法关掉。
- **复杂概念 / 方法要可视化**：遇到不好懂的概念、机制、流程、架构、数据关系、多方案对比时，不要只堆大段文字解释——**配一张图把它讲清楚**：内联 SVG 流程图 / 架构图、分步骤示意、对比表、时间线、带标注的简图等，让读者一眼看懂结构与关系。图一律用内联 SVG 或纯 HTML/CSS 画（不许外部图库 / 图片 URL）；文字负责补充，图负责让人「秒懂」。
- **数学公式用 HTML 排得好看**：要展示数学公式时**不能引 MathJax / KaTeX 等外部库**（沙箱会拦掉），用纯 HTML/CSS 自己排：`<sup>` / `<sub>` 处理上下标，衬线字体 + 合适字号排变量与算子，用 flex / grid 对齐分式、根号、求和、矩阵，符号用 Unicode（∑ ∫ √ ≈ ≤ ≥ × ÷ · ± ∂ ∇ ∈ → 等）。公式要对齐、清晰、有呼吸感，像印刷体排版，而不是挤成一行的纯文本；重要公式可单独放进一个浅色卡片里突出。
- **气质**：留白充足，视觉层级靠字号 / 字重 / 间距而不是密集的分割线；hover 强调用 `2px solid #d97757` 的 outline。整体像一份精排的印刷品——温暖、安静、克制。"""


_STYLE_REFERENCE_BLOCK = """下面是一篇**风格参考文档**——它本身就是一篇合格的 Phial 文档，演示了默认设计语言：色板、字体、`.tldr` 速读卡、折叠块、提示框、胶囊标签、代码块、`window.phial` 接入。

新建文档时**照这套视觉与结构来**：沿用同样的 `:root` 变量、同样的组件样式与圆角，把内容换成用户真正要的。这只是风格样板——**不要照抄它的文字内容**，只借它的外观与骨架。
...
def build_messages(
    ...
    editing = bool(current_html and current_html.strip())
    if editing:
    ...
    # On a fresh document the model has nothing to anchor the visual style to,
    # so ship a concrete in-style reference. When editing, the current doc is
    # itself the style anchor — skip the reference to keep the prompt lean.
    if not editing:
        messages.append({
            "role": "system",
            "content": _STYLE_REFERENCE_BLOCK.format(reference=REFERENCE_DOC.strip()),
        })
    if context_bundle and context_bundle.strip():
        messages.append({
            "role": "system",
